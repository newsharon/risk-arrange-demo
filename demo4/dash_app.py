...
# ── 재물 언더라이팅: 물건정보(화보협 스타일) / 누적조회 더미 데이터 ──────────
REGION_ADDR_BASE = {
    "충남": "충청남도 당진시 송악읍 한진리",
    "전남": "전라남도 여수시 율촌면 신풍리",
    "충북": "충청북도 청주시 청원구 오창읍 양청리",
    "울산": "울산광역시 남구 매암동",
    "전북": "전라북도 군산시 비응도동",
    "경남": "경상남도 창원시 성산구 귀곡동",
    "경기": "경기도 평택시 포승읍 신영리",
...
PROPERTY_STRUCTURES = ["철근콘크리트조(RC)", "철골조(S)", "철골철근콘크리트조(SRC)", "경량철골조"]
PROPERTY_USAGES = ["공장", "창고", "부속사무동"]
FIRE_FACILITY_GRADES = ["1급(스프링클러+옥내소화전)", "2급(옥내소화전)", "3급(소화기 비치)"]
HWABOHYEOP_GRADES = ["A", "B", "C", "D"]


def _property_seed(pid: int) -> Dict[str, Any]:
    """pid 기반 의사난수 건물정보(화보협 자료 흉내) 생성"""
    return {
        "structure": PROPERTY_STRUCTURES[pid % len(PROPERTY_STRUCTURES)],
        "usage": PROPERTY_USAGES[pid % len(PROPERTY_USAGES)],
        "floors_above": 2 + (pid % 5),
        "floors_below": pid % 2,
        "built_year": 1995 + (pid * 3) % 28,
        "total_area_m2": 8000 + (pid * 1737) % 42000,
        "fire_grade": FIRE_FACILITY_GRADES[pid % len(FIRE_FACILITY_GRADES)],
        "risk_grade": HWABOHYEOP_GRADES[pid % len(HWABOHYEOP_GRADES)],
    }


# 누적조회 데모용 — 주요 공장 인근에 배치되는 위성 물건(반경 1km 이내)
NEIGHBOR_CLUSTERS: Dict[int, List[str]] = {
    1:  ["삼우물류 당진센터", "동부제철 가공공장", "당진 컨테이너야드", "혜성스틸 가공센터"],
    4:  ["한화토탈 울산물류", "고려아연 온산공장", "에쓰오일 온산저장소", "성보화학 울산공장"],
    11: ["평택항 자동차물류센터", "기아 평택부품창고", "동희산업 평택공장", "평택 콜드체인센터"],
    21: ["부산항 컨테이너CY", "동성화학 부산공장", "부산 철강유통센터", "감천항 냉동창고"],
    2:  ["여수산단 저장탱크", "남해화학 여수공장", "여천NCC 부생창고", "여수 물류단지"],
}
NEIGHBOR_OFFSETS = [(0.003, 0.004), (-0.004, 0.002), (0.002, -0.005), (-0.003, -0.003)]

PROPERTY_INFO: List[Dict[str, Any]] = []
for f in DUMMY_FACTORIES:
    seed = _property_seed(f["factory_id"])
    PROPERTY_INFO.append({
        "property_id": f"PR-{f['factory_id']:04d}",
        "property_name": f["factory_name"],
        "address": f"{REGION_ADDR_BASE.get(f['region'], f['region'])} {100 + f['factory_id']}-{f['factory_id']}",
        "lat": f["lat"], "lon": f["lon"],
...
for anchor_id, names in NEIGHBOR_CLUSTERS.items():
    anchor = next(f for f in DUMMY_FACTORIES if f["factory_id"] == anchor_id)
    for i, name in enumerate(names):
        dlat, dlon = NEIGHBOR_OFFSETS[i]
        pid = 100 + anchor_id * 10 + i
        seed = _property_seed(pid)
        PROPERTY_INFO.append({
            "property_id": f"PR-{pid:04d}",
            "property_name": name,
            "address": f"{REGION_ADDR_BASE.get(anchor['region'], anchor['region'])} {200 + pid}-{i + 1}",
...
# 물건별 특이사항 (화보협/실사 코멘트 흉내)
PROPERTY_SPECIAL_NOTES: Dict[str, str] = {
    "PR-0008": "최근 3년 내 화재사고 이력 1건 (2024년, 경미)",
    "PR-0019": "인접 사업장과 동일 산업단지 내 위치 — 화재 시 동시 피해 가능성",
    "PR-0023": "해안 인접 — 태풍/침수 위험 가중",
    "PR-1042": "고압가스 저장소 인접 (반경 200m 이내)",
    "PR-0214": "스프링클러 미설치 구역 존재",
}

# 재물보험 LOB별 평균 요율 (가입금액 대비, 보험료 산출용)
LOB_RATE = {"기업재산보험": 0.0009, "화재보험": 0.0012, "배상책임보험": 0.0006, "특종보험": 0.0008}

PROPERTY_CONTRACTS: List[Dict[str, Any]] = []
for c in DUMMY_CONTRACTS_FACTORY:
    factory = next(f for f in DUMMY_FACTORIES if f["factory_id"] == c["factory_id"])
    pml_pct = 35 + (c["factory_id"] * 7) % 35
    rate = LOB_RATE.get(c["lob"], 0.0008)
    PROPERTY_CONTRACTS.append({
        "property_id": f"PR-{c['factory_id']:04d}",
        "contract_id": c["contract_id"],
        "policyholder": factory["factory_name"].split()[0],
        "lob": c["lob"], "product_name": c["product_name"],
        "insured_amount_krw": c["insured_amount_krw"],
        "pml_pct": pml_pct,
        "pml_krw": int(c["insured_amount_krw"] * pml_pct / 100),
        "premium_krw": int(c["insured_amount_krw"] * rate),
        "expiry": c["expiry"],
        "special_note": PROPERTY_SPECIAL_NOTES.get(f"PR-{c['factory_id']:04d}", ""),
    })

# 인근 위성 물건 중 일부에만 당사계약 보유 (나머지는 "당사계약 없음" 데모)
NEIGHBOR_CONTRACTS_EXTRA = [
    {"property_id": "PR-1010", "contract_id": "TCS-2026-PR-00510", "policyholder": "삼우물류", "lob": "기업재산보험", "product_name": "창고 종합재산보험", "insured_amount_krw": 8_000_000_000, "expiry": "2026-09-30"},
    {"property_id": "PR-1042", "contract_id": "TCS-2026-FI-00542", "policyholder": "한화토탈", "lob": "화재보험", "product_name": "화재/폭발 패키지", "insured_amount_krw": 32_000_000_000, "expiry": "2027-01-31"},
    {"property_id": "PR-1110", "contract_id": "TCS-2026-PR-00611", "policyholder": "동희산업", "lob": "기업재산보험", "product_name": "종합재산보험(All Risk)", "insured_amount_krw": 14_000_000_000, "expiry": "2026-12-31"},
    {"property_id": "PR-1112", "contract_id": "TCS-2026-SP-00612", "policyholder": "동희산업", "lob": "특종보험", "product_name": "기계보험", "insured_amount_krw": 6_000_000_000, "expiry": "2027-03-31"},
    {"property_id": "PR-0214", "contract_id": "TCS-2026-FI-00714", "policyholder": "동성화학", "lob": "화재보험", "product_name": "화재보험 단독", "insured_amount_krw": 11_000_000_000, "expiry": "2026-08-31"},
    {"property_id": "PR-0021", "contract_id": "TCS-2026-LI-00721", "policyholder": "동성화학", "lob": "배상책임보험", "product_name": "환경배상책임", "insured_amount_krw": 4_000_000_000, "expiry": "2027-02-28"},
    {"property_id": "PR-0040", "contract_id": "TCS-2026-PR-00840", "policyholder": "남해화학", "lob": "기업재산보험", "product_name": "종합재산보험(All Risk)", "insured_amount_krw": 26_000_000_000, "expiry": "2027-05-31"},
    {"property_id": "PR-0042", "contract_id": "TCS-2026-FI-00842", "policyholder": "여천NCC", "lob": "화재보험", "product_name": "화재/침수 패키지", "insured_amount_krw": 19_000_000_000, "expiry": "2026-11-30"},
]
for c in NEIGHBOR_CONTRACTS_EXTRA:
    rate = LOB_RATE.get(c["lob"], 0.0008)
    pml_pct = 35 + (int(c["property_id"].split("-")[1]) * 7) % 35
    PROPERTY_CONTRACTS.append({
        **c,
        "pml_pct": pml_pct,
        "pml_krw": int(c["insured_amount_krw"] * pml_pct / 100),
        "premium_krw": int(c["insured_amount_krw"] * rate),
        "special_note": PROPERTY_SPECIAL_NOTES.get(c["property_id"], ""),
    })


def build_property_df() -> pd.DataFrame:
    df = pd.DataFrame(PROPERTY_INFO)
    contracts = pd.DataFrame(PROPERTY_CONTRACTS)
    primary = (
        contracts.sort_values("insured_amount_krw", ascending=False)
        .drop_duplicates("property_id")[["property_id", "lob", "policyholder"]]
        .rename(columns={"lob": "primary_lob", "policyholder": "primary_policyholder"})
    )
    df = df.merge(primary, on="property_id", how="left")
    df["has_contract"] = df["primary_lob"].notna()
    return df


PROPERTY_DF = build_property_df()
PROPERTY_CONTRACTS_DF = pd.DataFrame(PROPERTY_CONTRACTS)
PROPERTY_RADIUS_OPTIONS = [250, 500, 1000]

# ── 재물 언더라이팅 내부 가이드라인 (더미 기준값) ──────────────────────────────
RISK_GRADE_WARNING = {"C", "D"}                # 화보협 위험등급 중 주의가 필요한 등급
RETENTION_PML_LIMIT_KRW = 15_000_000_000       # 반경 내 누적 PML 기준 보유 한도 (150억)
RETENTION_MIN_RATIO = 10                       # 최소 권장 보유율(%)
REGION_PML_GUIDELINE_KRW = 50_000_000_000      # 지역(시/도)별 누적 PML 가이드라인 한도 (500억)


def _circle_coords(lat: float, lon: float, radius_m: float, n: int = 60) -> Tuple[List[float], List[float]]:
    """지도에 원(반경)을 그리기 위한 좌표 목록 (위경도 근사)"""
    lats, lons = [], []
    for i in range(n + 1):
        ang = 2 * pi * i / n
        dlat = (radius_m / 1000) / 111.0 * cos(ang)
        dlon = (radius_m / 1000) / (111.0 * cos(radians(lat))) * sin(ang)
    ...
    html.Div([
        dbc.Nav([
            ...
            dbc.NavLink([html.I(className="fa-solid fa-calendar-days me-2"), "위험 캘린더"],
                        ...
                        style={"color": "#b0c4d8", "fontWeight": "600", "padding": "10px 24px"}),
            dbc.NavLink([html.I(className="fa-solid fa-magnifying-glass-location me-2"), "재물 UW 조회"],
                        href="/property", active="exact",
                        style={"color": "#b0c4d8", "fontWeight": "600", "padding": "10px 24px"}),
...
def _build_property_map(lob_filter: str, policyholder_filter: str, selected_id: Optional[str], radius_m: int,
    ...
    if policyholder_filter and policyholder_filter != "전체":
        df = df[df["primary_policyholder"] == policyholder_filter]

    if lob_filter and lob_filter != "전체":
        if lob_filter == "당사계약 없음":
            df = df[~df["has_contract"]]
        else:
            df = df[df["primary_lob"] == lob_filter]

    groups: Dict[str, List[pd.Series]] = {}
    for _, r in df.iterrows():
        lob = r["primary_lob"] if r["has_contract"] else "당사계약 없음"
        groups.setdefault(lob, []).append(r)

    for lob, rows in groups.items():
        color = LOB_COLORS.get(lob, PROPERTY_NO_CONTRACT_COLOR)
        lats, lons, texts, ids = [], [], [], []
        for r in rows:
            lats.append(r["lat"])
            lons.append(r["lon"])
            contract_txt = (
                f"계약자: {r['primary_policyholder']}<br>보종: {r['primary_lob']}"
                if r["has_contract"] else "당사계약 없음 (물건정보만 보유)"
            )
            texts.append(f"<b>{r['property_name']}</b><br>{r['address']}<br>{contract_txt}")
            ids.append(r["property_id"])
        fig.add_trace(go.Scattermapbox(
            lat=lats, lon=lons, mode="markers",
            marker=dict(size=13, color=color, opacity=0.85),
            text=texts, hoverinfo="text", customdata=ids, name=lob,
        ))

    center = dict(lat=36.0, lon=128.0)
    zoom = 6.2

    if selected_id:
        sel = PROPERTY_DF[PROPERTY_DF["property_id"] == selected_id]
        if not sel.empty:
            srow = sel.iloc[0]
            center = dict(lat=srow["lat"], lon=srow["lon"])
            zoom = 12
            fig.add_trace(go.Scattermapbox(
                lat=[srow["lat"]], lon=[srow["lon"]], mode="markers",
                marker=dict(size=22, color="#1a2942", opacity=0.35),
                hoverinfo="skip", showlegend=False,
            ))
            clats, clons = _circle_coords(srow["lat"], srow["lon"], radius_m)
            fig.add_trace(go.Scattermapbox(
                lat=clats, lon=clons, mode="lines",
                line=dict(width=2, color="#e74c3c"),
                fill="toself", fillcolor="rgba(231,76,60,0.08)",
                hoverinfo="skip", showlegend=False, name=f"반경 {radius_m}m",
            ))
...
def _build_property_detail(selected_id: Optional[str]) -> html.Div:
    if not selected_id:
        return dbc.Alert(
            "지도에서 물건을 선택하거나, 좌측 검색창에 소재지/건물명을 입력해 조회하세요.",
            color="secondary", style={"borderRadius":"10px","fontSize":"0.85rem"},
        )

    prow_df = PROPERTY_DF[PROPERTY_DF["property_id"] == selected_id]
    if prow_df.empty:
        return dbc.Alert("물건 정보를 찾을 수 없습니다.", color="warning")
    p = prow_df.iloc[0]

    risk_grade = p["risk_grade"]
    if risk_grade in RISK_GRADE_WARNING:
        risk_grade_cell: Any = html.Span(
            [risk_grade, " (주의등급)"],
            style={"background":"#e74c3c","color":"#fff","borderRadius":"4px",
                   "padding":"1px 8px","fontWeight":"700"},
        )
    else:
        risk_grade_cell = html.Span(
            risk_grade,
            style={"background":"#27ae60","color":"#fff","borderRadius":"4px",
                   "padding":"1px 8px","fontWeight":"700"},
        )
    ...
    info_rows: List[Tuple[str, Any]] = [
        ...
        ("건물 구조", p["structure"]),
        ("용도", p["usage"]),
        ("규모", f"지상 {p['floors_above']}층 / 지하 {p['floors_below']}층"),
        ("연면적", f"{p['total_area_m2']:,} ㎡"),
    ...
    info_table = dbc.Table(
        html.Tbody([
            html.Tr([
                html.Td(k, style={"fontSize":"0.8rem","fontWeight":"700","color":"#555","width":"38%"}),
    ...
    note = PROPERTY_SPECIAL_NOTES.get(selected_id, "")
    blocks: List[Any] = [section_header(f"물건정보 — {p['property_name']}"), info_table]
    if note:
        blocks.append(dbc.Alert(
            [html.I(className="fa-solid fa-circle-exclamation me-2"), note],
            color="warning", style={"fontSize":"0.8rem","padding":"8px 12px","marginTop":"8px"},
        ))

    blocks.append(html.Hr(style={"margin":"10px 0"}))

    contracts = PROPERTY_CONTRACTS_DF[PROPERTY_CONTRACTS_DF["property_id"] == selected_id]
    if contracts.empty:
        blocks.append(section_header("당사 계약정보"))
        blocks.append(dbc.Alert([
            html.I(className="fa-solid fa-circle-info me-2"),
            "당사계약 없음 — 화보협 물건정보만 조회되었습니다. 신규 계약 인수 검토 시 참고하세요.",
        ], color="info", style={"fontSize":"0.82rem","borderRadius":"8px"}))
    else:
        blocks.append(section_header(f"당사 계약정보 — {len(contracts)}건"))
        contract_cards = []
        for _, c in contracts.iterrows():
            lob_color = LOB_COLORS.get(c["lob"], "#888")
            contract_cards.append(html.Div([
                html.Div([
                    html.Span(c["lob"], style={"background":lob_color,"color":"#fff","borderRadius":"4px",
                              "padding":"1px 8px","fontSize":"0.72rem","fontWeight":"700","marginRight":"8px"}),
                    html.Span(c["contract_id"], style={"fontFamily":"monospace","fontSize":"0.78rem","fontWeight":"700"}),
                ], style={"marginBottom":"4px"}),
                html.Div(f"계약자: {c['policyholder']} · {c['product_name']}", style={"fontSize":"0.8rem","color":"#444"}),
                html.Div(
                    f"가입금액 {c['insured_amount_krw']/1e8:.0f}억 · PML {c['pml_pct']}% ({c['pml_krw']/1e8:.1f}억) · 당사보험료 {c['premium_krw']/1e4:.0f}만원",
                    style={"fontSize":"0.8rem","color":"#2980b9","fontWeight":"600"},
                ),
                html.Div(f"만기 {c['expiry']}", style={"fontSize":"0.76rem","color":"#888"}),
            ], style={"padding":"10px 0","borderBottom":"1px solid #f0f0f0"}))
        blocks.append(html.Div(contract_cards))

    return html.Div(blocks)


def _build_accum_banner(selected_id: Optional[str], radius_m: int) -> html.Div:
    if not selected_id:
        return html.Div()
    ...
    nearby = []
    for _, r in PROPERTY_DF.iterrows():
        dist_km = haversine_km(p["lat"], p["lon"], r["lat"], r["lon"])
        if dist_km <= radius_km:
            nearby.append((r, dist_km))
    nearby.sort(key=lambda x: x[1])

    total_amount = total_pml = total_premium = 0
    n_contracts = 0
    table_rows = []
    for r, dist_km in nearby:
        contracts = PROPERTY_CONTRACTS_DF[PROPERTY_CONTRACTS_DF["property_id"] == r["property_id"]]
        if contracts.empty:
            table_rows.append(html.Tr([
                html.Td(r["property_name"], style={"fontSize":"0.78rem","fontWeight":"600"}),
                html.Td(f"{dist_km*1000:.0f}m", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Td("-", style={"fontSize":"0.78rem","textAlign":"center","color":"#aaa"}),
                html.Td("-", style={"fontSize":"0.78rem","textAlign":"center","color":"#aaa"}),
                html.Td("-", style={"fontSize":"0.78rem","textAlign":"right","color":"#aaa"}),
                html.Td("-", style={"fontSize":"0.78rem","textAlign":"right","color":"#aaa"}),
                html.Td("-", style={"fontSize":"0.78rem","textAlign":"right","color":"#aaa"}),
                html.Td("당사계약 없음", style={"fontSize":"0.76rem","color":"#aaa"}),
            ]))
            continue
        for _, c in contracts.iterrows():
            n_contracts += 1
            total_amount += c["insured_amount_krw"]
            total_pml += c["pml_krw"]
            total_premium += c["premium_krw"]
            lob_color = LOB_COLORS.get(c["lob"], "#888")
            table_rows.append(html.Tr([
                html.Td(r["property_name"], style={"fontSize":"0.78rem","fontWeight":"600"}),
                html.Td(f"{dist_km*1000:.0f}m", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Td(c["policyholder"], style={"fontSize":"0.78rem","textAlign":"center"}),
                html.Td(html.Span(c["lob"], style={"background":lob_color,"color":"#fff","borderRadius":"4px",
                        "padding":"1px 7px","fontSize":"0.7rem","fontWeight":"600"}), style={"textAlign":"center"}),
                html.Td(f"{c['insured_amount_krw']/1e8:.0f}억", style={"fontSize":"0.78rem","textAlign":"right","color":"#2980b9","fontWeight":"700"}),
                html.Td(f"{c['pml_krw']/1e8:.1f}억", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Td(f"{c['premium_krw']/1e4:.0f}만", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Td(c["special_note"] or "-", style={"fontSize":"0.74rem","color":"#e67e22"}),
            ]))

    # 반경 내 화보협 위험등급 C/D(주의등급) 물건 수 — 위험 집중도 분석
    n_risk_grade_warning = sum(1 for r, _ in nearby if r["risk_grade"] in RISK_GRADE_WARNING)

    summary_cards = dbc.Row([
        dbc.Col(kpi_card("반경 내 물건",   f"{len(nearby)}건",              "#1a2942", "fa-map-pin"),            md=3),
        dbc.Col(kpi_card("누적 계약건수",  f"{n_contracts}건",              "#2980b9", "fa-file-contract"),       md=3),
        dbc.Col(kpi_card("누적 가입금액",  f"{total_amount/1e8:.0f}억 원",  "#27ae60", "fa-won-sign"),            md=3),
        dbc.Col(kpi_card("누적 PML",       f"{total_pml/1e8:.0f}억 원",     "#e74c3c", "fa-triangle-exclamation"), md=3),
    ], className="mb-2")

    # ── 주요 위험 분석 요약 ──────────────────────────────────────────────────
    risk_items: List[html.Li] = []
    if p["risk_grade"] in RISK_GRADE_WARNING:
        risk_items.append(html.Li(f"해당 물건 화보협 위험등급 '{p['risk_grade']}' (주의등급) — 인수 시 위험도 검토 필요"))
    if n_risk_grade_warning:
        risk_items.append(html.Li(f"반경 {radius_m}m 내 위험등급 C/D(주의등급) 물건 {n_risk_grade_warning}건 — 동시 손해 위험 집중 가능성"))
    own_note = PROPERTY_SPECIAL_NOTES.get(p["property_id"], "")
    if own_note:
        risk_items.append(html.Li(f"해당 물건 특이사항: {own_note}"))
    other_notes = [(r["property_name"], r["special_note"]) for r, _ in nearby
                    if r["property_id"] != p["property_id"]
                    and PROPERTY_SPECIAL_NOTES.get(r["property_id"])]
    for name, note_txt in other_notes:
        risk_items.append(html.Li(f"인근 '{name}' 특이사항: {note_txt}"))

    risk_block = dbc.Alert(
        [html.Div([html.I(className="fa-solid fa-magnifying-glass-chart me-2"), html.B("주요 위험 분석")],
                  style={"marginBottom":"4px"}),
         html.Ul(risk_items, style={"marginBottom":0, "paddingLeft":"20px", "fontSize":"0.8rem"})]
        if risk_items else
        [html.I(className="fa-solid fa-circle-check me-2"), "반경 내 특이 위험요소가 확인되지 않았습니다."],
        color="warning" if risk_items else "success",
        style={"fontSize":"0.82rem","borderRadius":"8px","marginBottom":"10px"},
    )

    # ── 보유율 가이드 ────────────────────────────────────────────────────────
    if total_pml > 0:
        recommended_ratio = max(RETENTION_MIN_RATIO, min(100, round(RETENTION_PML_LIMIT_KRW / total_pml * 100)))
    else:
        recommended_ratio = 100
    over_guideline = total_pml > RETENTION_PML_LIMIT_KRW

    retention_block = dbc.Alert([
        html.Div([html.I(className="fa-solid fa-scale-balanced me-2"), html.B("보유율 가이드 (참고용)")],
                 style={"marginBottom":"4px"}),
        html.Div(f"반경 내 누적 PML {total_pml/1e8:.1f}억 원 / 사내 보유 한도 {RETENTION_PML_LIMIT_KRW/1e8:.0f}억 원",
                 style={"fontSize":"0.8rem"}),
        html.Div([
            "권장 보유율: ",
            html.B(f"{recommended_ratio}%"),
            "  —  ",
            html.Span("가이드라인 초과 (재보험 출재 비중 확대 검토)" if over_guideline else "정상 범위",
                      style={"color":"#e74c3c" if over_guideline else "#27ae60", "fontWeight":"700"}),
        ], style={"fontSize":"0.82rem","marginTop":"4px"}),
    ], color="danger" if over_guideline else "info",
       style={"fontSize":"0.82rem","borderRadius":"8px","marginBottom":"10px"})

    return dbc.Card(dbc.CardBody([
        section_header(f"누적조회 — '{p['property_name']}' 반경 {radius_m}m 이내"),
        summary_cards,
        risk_block,
        retention_block,
        html.Div(f"누적 당사보험료 합계: {total_premium/1e4:,.0f}만 원",
                 style={"fontSize":"0.85rem","fontWeight":"700","color":"#1a2942","marginBottom":"8px"}),
        dbc.Table([
            html.Thead(html.Tr([
                html.Th("물건명", style={"fontSize":"0.78rem"}),
                html.Th("거리", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Th("계약자", style={"fontSize":"0.78rem","textAlign":"center"}),
                html.Th("보종", style={"fontSize":"0.78rem","textAlign":"center"}),
                html.Th("가입금액", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Th("PML", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Th("당사보험료", style={"fontSize":"0.78rem","textAlign":"right"}),
                html.Th("특이사항", style={"fontSize":"0.78rem"}),
            ], style={"background":"#f8f9fa"})),
...
def layout_property() -> html.Div:
    policyholders = ["전체"] + sorted(PROPERTY_CONTRACTS_DF["policyholder"].unique().tolist())
    available_lobs = [lob for lob in LOB_COLORS if lob in PROPERTY_CONTRACTS_DF["lob"].unique()]

    lob_filter_buttons = dbc.ButtonGroup([
        dbc.Button("전체", id={"type":"prop-lob-btn","index":"전체"}, size="sm", outline=True, color="secondary",
                   style={"fontWeight":"700","fontSize":"0.78rem"}),
        *[dbc.Button(lob, id={"type":"prop-lob-btn","index":lob}, size="sm", outline=True,
                     style={"fontWeight":"600","fontSize":"0.78rem","color":LOB_COLORS[lob],
                            "borderColor":LOB_COLORS[lob]})
          for lob in available_lobs],
        dbc.Button("당사계약 없음", id={"type":"prop-lob-btn","index":"당사계약 없음"}, size="sm", outline=True,
                   style={"fontWeight":"600","fontSize":"0.78rem","color":PROPERTY_NO_CONTRACT_COLOR,
                          "borderColor":PROPERTY_NO_CONTRACT_COLOR}),
    ], style={"flexWrap":"wrap","gap":"4px"})

    return html.Div([
        dcc.Store(id="prop-selected-id", data=None),
        dcc.Store(id="prop-lob-filter", data="전체"),

        html.Div([
            html.H4("재물 UW 조회", style={"fontWeight":"800","color":"#1a2942","margin":0}),
            html.Div("소재지 검색 → 당사계약/물건정보 조회 → 반경 내 누적현황 확인 (재물 언더라이팅)",
                     style={"color":"#888","fontSize":"0.82rem"}),
        ], style={"marginBottom":"16px"}),
        ...
        dbc.Row([
            dbc.Col([
                html.Div([
                    dcc.Input(
                        id="prop-search-input", type="text",
                        placeholder="소재지/건물명 검색 (예: 당진, 여수산단, 현대제철)",
                        debounce=False,
                        style={"flex":"1","padding":"7px 12px","borderRadius":"6px",
                               "border":"1px solid #ccc","fontSize":"0.85rem","marginRight":"8px"},
                    ),
                    dbc.Button([html.I(className="fa-solid fa-magnifying-glass me-1"), "검색"],
                               id="prop-search-btn", color="primary", size="sm",
                               style={"fontWeight":"700","whiteSpace":"nowrap"}),
                ], style={"display":"flex","alignItems":"center"}),
                html.Div(id="prop-search-result", style={"fontSize":"0.78rem","color":"#e74c3c","marginTop":"4px"}),
            ], md=7),
            dbc.Col([
                html.Div([
                    html.Span("누적조회 반경:", style={"fontSize":"0.83rem","fontWeight":"600","color":"#444","marginRight":"8px"}),
                    dcc.Dropdown(
                        id="prop-radius-select",
                        options=[{"label": f"{r}m", "value": r} for r in PROPERTY_RADIUS_OPTIONS],
                        value=500, clearable=False, style={"width":"140px","fontSize":"0.85rem"},
                    ),
                ], style={"display":"flex","alignItems":"center"}),
            ], md=5, style={"display":"flex","alignItems":"center","justifyContent":"flex-end"}),
        ], className="mb-2"),

        html.Div([
            html.Div([
                html.Span("보종 필터:", style={"fontSize":"0.83rem","fontWeight":"600","color":"#444","marginRight":"8px"}),
                lob_filter_buttons,
            ], style={"display":"flex","alignItems":"center","flexWrap":"wrap","gap":"4px","marginBottom":"8px"}),
            html.Div([
                html.Span("계약자 필터:", style={"fontSize":"0.83rem","fontWeight":"600","color":"#444","marginRight":"8px"}),
                dcc.Dropdown(
                    id="prop-policyholder-filter",
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Card(dbc.CardBody(html.Div(_build_property_detail(None), id="prop-detail")),
                         style={**CARD_STYLE, "maxHeight":"620px", "overflowY":"auto"}),
            ], md=4),
        ], className="mb-3"),

        html.Div(_build_accum_banner(None, 500), id="prop-accum-banner"),
    ])


@callback(
    Output("prop-selected-id", "data"),
    Output("prop-search-result", "children"),
    Input("prop-search-btn", "n_clicks"),
    Input("prop-map", "clickData"),
    State("prop-search-input", "value"),
    prevent_initial_call=True,
)
def select_property(search_clicks: Any, click_data: Optional[Dict], search_value: Optional[str]):
    ctx = dash.callback_context
    if not ctx.triggered:
        return dash.no_update, dash.no_update
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]

    if trigger_id == "prop-map":
        if not click_data:
            return dash.no_update, dash.no_update
        point = click_data["points"][0]
        pid = point.get("customdata")
        if pid:
            return pid, ""
        return dash.no_update, dash.no_update

    if trigger_id == "prop-search-btn":
        query = (search_value or "").strip()
        if not query:
            return dash.no_update, "검색어를 입력하세요."
        matched = PROPERTY_DF[
            PROPERTY_DF["property_name"].str.contains(query, case=False, na=False)
            | PROPERTY_DF["address"].str.contains(query, case=False, na=False)
        ]
        if matched.empty:
            return dash.no_update, f"'{query}'에 해당하는 물건/소재지를 찾을 수 없습니다."
        return matched.iloc[0]["property_id"], ""

    return dash.no_update, dash.no_update


@callback(
    Output("prop-lob-filter", "data"),
    Input({"type":"prop-lob-btn","index":dash.ALL}, "n_clicks"),
    prevent_initial_call=True,
)
def update_property_lob_filter(n_clicks_list: List):
    ctx = dash.callback_context
    if not ctx.triggered:
        return "전체"
    import json as _json
...
@callback(
    Output("prop-map", "figure"),
    Input("prop-lob-filter", "data"),
...
@callback(
    Output("prop-detail", "children"),
    Output("prop-accum-banner", "children"),
    Input("prop-selected-id", "data"),
    Input("prop-radius-select", "value"),
)
def update_property_detail(selected_id: Optional[str], radius_m: Optional[int]):
    return _build_property_detail(selected_id), _build_accum_banner(selected_id, radius_m or 500)


# ── 페이지 6: 지역별 누적현황 (내부 가이드라인 활용) ──────────────────────────
def build_region_stats_df() -> pd.DataFrame:
    merged = PROPERTY_CONTRACTS_DF.merge(
        PROPERTY_DF[["property_id", "region"]], on="property_id", how="left"
    )
    contract_stats = merged.groupby("region").agg(
        contract_count=("contract_id", "count"),
        total_amount_krw=("insured_amount_krw", "sum"),
        total_pml_krw=("pml_krw", "sum"),
        total_premium_krw=("premium_krw", "sum"),
    ).reset_index()
    ...
    for col in ["contract_count", "total_amount_krw", "total_pml_krw", "total_premium_krw"]:
...
REGION_STATUS_COLOR = {"초과": "#e74c3c", "주의": "#f39c12", "정상": "#27ae60"}


def _build_region_pml_chart(df: pd.DataFrame) -> go.Figure:
    colors = [REGION_STATUS_COLOR[s] for s in df["status"]]
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=df["region"], y=df["total_pml_krw"] / 1e8,
        marker_color=colors, name="누적 PML(억 원)",
        text=[f"{v/1e8:.0f}억" for v in df["total_pml_krw"]], textposition="outside",
    ))
    fig.add_hline(
        y=REGION_PML_GUIDELINE_KRW / 1e8, line_dash="dash", line_color="#e74c3c",
        annotation_text=f"가이드라인 한도 {REGION_PML_GUIDELINE_KRW/1e8:.0f}억",
        annotation_position="top right",
    )
    fig.update_layout(
        margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor="white", plot_bgcolor="white",
        yaxis_title="누적 PML (억 원)", height=380, showlegend=False,
    )
    return fig


def layout_property_region() -> html.Div:
    df = build_region_stats_df()
    ...
    for _, r in df.iterrows():
        ...
        table_rows.append(html.Tr([
            ...
            html.Td(f"{r['no_contract_count']}건", style={"fontSize":"0.8rem","textAlign":"right","color":"#888"}),
            html.Td(f"{r['contract_count']}건", style={"fontSize":"0.8rem","textAlign":"right"}),
            html.Td(f"{r['total_amount_krw']/1e8:.0f}억", style={"fontSize":"0.8rem","textAlign":"right","color":"#2980b9","fontWeight":"700"}),
    ...
    return html.Div([
        html.Div([
            html.H4("재물 UW 현황", style={"fontWeight":"800","color":"#1a2942","margin":0}),
            html.Div("지역(시/도)별 계약·누적 PML 현황 — 내부 가이드라인 대비 모니터링 (재물 언더라이팅)",
                     style={"color":"#888","fontSize":"0.82rem"}),
        ], style={"marginBottom":"16px"}),

        dbc.Row([
            dbc.Col(kpi_card("전체 지역 수", f"{len(df)}개", "#1a2942", "fa-map"), md=3),
        ...
        dbc.Card(dbc.CardBody([
            section_header("지역별 누적 PML — 가이드라인 한도 대비"),
            dcc.Graph(figure=_build_region_pml_chart(df), config={"displayModeBar": False}),
        ]), style={**CARD_STYLE, "marginBottom":"16px"}),

        dbc.Card(dbc.CardBody([
            section_header("지역별 상세 현황"),
            dbc.Table([
                html.Thead(html.Tr([
                    html.Th("지역", style={"fontSize":"0.78rem"}),
                    html.Th("물건 수", style={"fontSize":"0.78rem","textAlign":"right"}),
                    html.Th("당사계약 없음", style={"fontSize":"0.78rem","textAlign":"right"}),
                    html.Th("계약건수", style={"fontSize":"0.78rem","textAlign":"right"}),
                    html.Th("누적 가입금액", style={"fontSize":"0.78rem","textAlign":"right"}),
